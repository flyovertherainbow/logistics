...
PASSWORD = "Eclyltd88$"
CONTAINER_INPUT_SELECTOR_CANDIDATES = [
    '#container-input-textarea',
    '#txContainerInput',
    'textarea#container-input-textarea',
    'textarea#txContainerInput'
]
CONTAINER_INPUT_SELECTOR = CONTAINER_INPUT_SELECTOR_CANDIDATES[0]
...
def execute_login_sequence(page, USERNAME, PASSWORD, PORTCONNECT_URL, status_placeholder):
    ...
    try:
        ...
        # Click the search link; SPA may not trigger a full navigation
        try:
            with page.expect_navigation(timeout=5000):
                page.click(TRACK_AND_TRACE_SEARCH_LINK)
        except PlaywrightTimeoutError:
            # If no navigation event, still click and proceed to element wait
            page.click(TRACK_AND_TRACE_SEARCH_LINK)

        # Resolve and wait for the container input field using multiple selectors
        status_placeholder.info("Waiting for container input field to appear...")
        global CONTAINER_INPUT_SELECTOR
        resolved = None
        for sel in CONTAINER_INPUT_SELECTOR_CANDIDATES:
            try:
                page.wait_for_selector(sel, state="visible", timeout=25000)
                resolved = sel
                status_placeholder.info(f"Detected container input via selector: {sel}")
                break
            except PlaywrightTimeoutError:
                continue
        if not resolved:
            status_placeholder.warning("Container input not found by known selectors. Retrying...")
            page.wait_for_timeout(2000)
            for sel in CONTAINER_INPUT_SELECTOR_CANDIDATES:
                try:
                    ...
                    continue
        if not resolved:
            raise PlaywrightTimeoutError("Container input field did not appear after navigating to search page.")
        CONTAINER_INPUT_SELECTOR = resolved
...
# Container input selector is resolved at runtime using candidate selectors
...
def run_crawler(container_list, status_placeholder):
    ...
    try:
        with sync_playwright() as p:
            ...
            # --- 3. Perform Search ---
            SEARCH_BUTTON_SELECTOR = 'button.btn.btn-primary:has-text("Search")'
